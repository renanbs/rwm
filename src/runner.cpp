...
void Runner::load_cmd_history()
{
    ...
    run_history = new QFile(QDir::tempPath() + "/rwm-runner.log");
    // create it if doesn't exists
    if(!run_history->exists())
    QProcess::startDetached(QString("/bin/touch").append(" ").append(QDir::tempPath() + "/rwm-runner.log"));

    if (!run_history->open(QIODevice::ReadOnly))
    {
        qCritical() << "Error on opening <rwm-runner.log> file";
        return;
    }
