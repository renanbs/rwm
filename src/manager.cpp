...
void Manager::set_run_args(QTableWidgetItem *item) // to set run arguments for startup applications
{
    ...
    rwm->beginGroup(name_item->text()); // application name
    rwm->setValue("args", item->text()); // application args on startup
    rwm->endGroup(); // Name
...
{
    //////////////////////////////////////////////////////////////////
    rwm->beginGroup("Style"); // save the style
    rwm->setValue("name", stl_name);
    rwm->setValue("path", stl_path);
    rwm->endGroup(); //Style
    //////////////////////////////////////////////////////////////////
    style->beginGroup("Frame");
    style->beginGroup("Border");
    ...
    Msgbox msg;
    msg.set_header(tr("RWM STYLE SETTINGS UPDATED"));
...
void Manager::add_app_pressed() // add selected app on lancher menu (in the select Category)
{
    if (tree_view->selectionModel()->currentIndex().isValid())
    {
        ...
        if ( ! app.isEmpty() && info.isExecutable() && ! info.isDir())
        {
            ...
            rwm->beginGroup("Launcher");
            rwm->beginGroup("Category");
            rwm->beginGroup(cat_map.value(category_combo->currentText())); // the select Category type from combobox (with translation)
            rwm->beginGroup(app);

            rwm->setValue("name", app);
            rwm->setValue("exec", path);
            rwm->setValue("pix", icon);
            rwm->endGroup(); // App name
            rwm->endGroup(); // Category type
            rwm->endGroup(); // Category group
            rwm->endGroup(); // Launcher
            update_remove_list();
            Msgbox msg;
...
void Manager::add_run_app_pressed() // add selected app on "Run at startup" list
{
    ...
    if (file_dialog->exec() == QDialog::Accepted)
    {
        ...
        if (! name.isEmpty() && pathinfo.isExecutable())
        {
            ...
            rwm->setValue("path", path);
            rwm->setValue("args", ""); // for future settings
            rwm->endGroup(); // Name
            rwm->endGroup(); // Startup
        }
    }
...
void Manager::remove_run_app_pressed() // remove selected app from "Run at startup" list
{
    ...
    if (current_row >= 0) // if some row is selected
    {
        ...
        rwm->beginGroup("Startup");
        rwm->remove(name_item->text()); // remove the application
        rwm->endGroup(); // Startup
...
void Manager::remove_app_pressed() // remove selected app from Launcher menu
{
    if (app_tree->currentItem() != NULL && app_tree->currentItem()->parent() != NULL) // remove only is item is selected (no if it's a Category)
    {
        ...
        rwm->beginGroup("Launcher");
        rwm->beginGroup("Category");
        rwm->beginGroup(cat_group); // Category group (parent of application)
        qDebug() << "Parent Application:" << cat_group;
        rwm->remove(app); // remove the application
        rwm->endGroup(); // Category group
        rwm->endGroup(); // Category
        rwm->endGroup(); // Launcher
        update_remove_list();
        Msgbox msg;
...
void Manager::update_remove_list() // update the Category/Apps list on remove tab
{
    ...
    for (int c = 0; c < rwm->childGroups().size(); c++)
    {
        QString cat_name = rwm->childGroups().value(c); //Category group (Utility, Office ...)
        rwm->beginGroup(cat_name); // Category group
        ...
        {
            QString app_name = rwm->childGroups().value(i); //Application name
...
void Manager::update_run_table() // update the "Run at startup" list
{
    ...
    for (int i = 0; i < rwm->childGroups().size(); i++)
    {
        ...
        QString path = rwm->value("path").toString(); // App path
        QString args = rwm->value("args").toString(); // App arguments
        Appicon app_icon; // get application icon
        QString icon = app_icon.get_app_icon(app_name);
