...
void Rwm::create_frame(Window c_win, Dockbar *dock, Desk *desk) // create new frame around the client app
{
...
bool Rwm::check_net_sys_tray_for(Window c_win) const
{
    Atom type_ret = None;
...
void Rwm::check_window_type(Window c_win) // chech the window type before mapping
{
    Atom type_ret = None;
    Atom *win_type;
    unsigned char *data = 0;
...
void Rwm::check_wm_transient_for(Window c_win)
{
    Atom type_ret = None;
...
void Rwm::print_window_prop(Window c_win) // print the window properties
{
    int num_prop_ret = 0;
...
void Rwm::raise_next_frame() // raise next frame on [Alt+Tab] key combination
{
...
void Rwm::set_active_frame(Frame *frm) // activation of selected frame and unactivation of others
{
...
void Rwm::send_configurenotify(Frame *frm)
{
    XConfigureEvent ce;
...
void Rwm::wm_quit()
{
    ...
    msg.set_info(tr("Are you sure to quit the RWM ?"));
...
//    rwm = new QSettings(QCoreApplication::applicationDirPath() + "/rwm.cfg", QSettings::IniFormat, this);
    ...
    {
        QStringList args;
        QString app_name = rwm->childGroups().value(i);
        rwm->beginGroup(app_name); // App group
        QString name = rwm->value("name").toString();
        QString path = rwm->value("path").toString();
        args << rwm->value("args").toString();
        QProcess::startDetached(path + name, args); // run the application + args
        rwm->endGroup(); // Name
    }
    rwm->endGroup(); // Startup
}
...
Desk * Rwm::get_desktop()
{
    return dsk;
}

Dockbar * Rwm::get_dockbar()
{
    return dock;
}

Categorymenu * Rwm::get_category_menu()
{
    return cat_menu;
}

void Rwm::testAndCopyDirs()
{
	QDir dir;
	bool exist = false;
	QString directory;
	...
	{
		directory = d.at(i).toLocal8Bit().constData();
	...
	if (exist)
	{
		directory = QDir::homePath() + Paths::getPath();
		dir.setPath(directory);
		if (!dir.exists())
		{
			...
			{
				// Copy files...
				copyDir ("/usr/share/rwm/docs", "/home/renan/.local/share/rwm/docs");
				copyDir ("/usr/share/rwm/themes", "/home/renan/.local/share/rwm/themes");
				copyDir ("/usr/share/rwm/translations", "/home/renan/.local/share/rwm/translations");
				copyDir ("/usr/share/rwm/images", "/home/renan/.local/share/rwm/images");
			}
			else
			{
			...
			// FAZER O TESTE (1) AQUI
			qDebug() << "Still need to test if the right files are in the right place, but for now I will assume they are!";
	}
	else
		qFatal ("Please reinstall RWM, there are missing files in your installation...");
}

void Rwm::copyDir (QString sourceFolder, QString destFolder)
{
...
void Rwm::createRWMconfig()
{
	QString directory = Paths::getConfigPath();
	QFile file(directory + "rwm.cfg");
	if (!file.open(QIODevice::WriteOnly | QIODevice::Text))
		qFatal ("Cannot open rwm.cfg for writing...");

	QTextStream out(&file);
	out << "[Style]\n";
	out << "name = default.stl\n";
	out << "path = " + Paths::getThemesPath() + "default/\n\n";  
}

void Rwm::setSettings()
{
	QDir dir;
	// Test if there is a the rwm.cfg file, if it doesn't, the other files do not exist as well
	QString directory = Paths::getConfigPath();
	dir.setPath(directory);
	if (dir.exists())
	{
		...
		if (file.exists())
		{
			rwm = new QSettings(directory + "/rwm.cfg", QSettings::IniFormat, this);
			...
			if (rwm->status() == QSettings::AccessError)
		...
		else
		{
			...
			out << "[Style]\n";
			out << "name = default.stl\n";
			out << "path = " + Paths::getConfigPath() + "\n\n";
		}
	}
	else
	{
		directory = QDir::homePath() + Paths::getConfigRwm();
		if (dir.mkdir(directory))
		{
			directory = Paths::getConfigPath();
			if (dir.mkdir(directory))
				createRWMconfig();
			else
			{
