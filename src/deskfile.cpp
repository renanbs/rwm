...
void Deskfile::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
    releaseMouse();
    // save new deskfile position on desk writing on rwm.cfg
    rwm->beginGroup("Desktop");
    rwm->beginGroup("File");
    rwm->beginGroup(file_name); // File name
...
void Deskfile::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (act != 0 && act != delete_file)
    {
        ...
        rwm->setValue("pix", icon); // replace the default pixmap
        rwm->endGroup(); // File name
        rwm->endGroup(); // File
        rwm->endGroup(); // Desktop
    }
