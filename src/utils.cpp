...
void Categorymenu::update_menu()
{
    ...
    {
        cat_iter.next();
        rwm->beginGroup(cat_iter.key()); // category name
        for (int i = 0; i < rwm->childGroups().size(); i++)
        {
            ...
            QString icon = rwm->value("pix").toString();
    ...
    {
        cat_desk_iter.next();
        rwm->beginGroup(cat_desk_iter.key()); // category name
        for (int i = 0; i < rwm->childGroups().size(); i++)
        {
            rwm->beginGroup(rwm->childGroups().value(i)); // App name
            QString name = rwm->value("name").toString();
...
{
    rwm->remove("Launcher-Desk"); // clear Launcher-Desk entry
    ...
    while (desktop_iter.hasNext())
    {
        ...
        if (! exec.isEmpty() && ! categories.isEmpty() && ! name.isEmpty() && ! icon_name.isEmpty())
        {
            ...
            rwm->beginGroup("Launcher-Desk");
            rwm->beginGroup("Category");
            rwm->beginGroup(categories);
            rwm->beginGroup(app_name + args); // for i.e. soffice-writer

            rwm->setValue("name", name);
            rwm->setValue("exec", full_path);
            rwm->setValue("pix", icon_path);
