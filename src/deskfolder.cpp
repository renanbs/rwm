...
void Deskfolder::mouseReleaseEvent(QMouseEvent *event)
{
    Q_UNUSED(event);
    releaseMouse();
    // save new deskfolder position on desk writing on rwm.cfg
    rwm->beginGroup("Desktop");
    rwm->beginGroup("Folder");
    rwm->beginGroup(dir_name); // Icon name
...
void Deskfolder::mouseDoubleClickEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton)
    {
        rwm->beginGroup("Desktop");
        rwm->beginGroup("Folder");
        rwm->beginGroup(dir_name); // Folder name
        ...
        else // open with Filedialog
        {
            ...
            if (file_dialog->exec() == QDialog::Rejected) // on close button
            {
                ...
                rwm->beginGroup("Desktop");
                rwm->beginGroup("Folder");
                rwm->beginGroup(dir_name);
                rwm->setValue("geometry", file_dialog->geometry());
                rwm->endGroup(); //name
                rwm->endGroup(); //Folder
                rwm->endGroup(); //Desktop
...
void Deskfolder::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (act != 0 && act != delete_folder)
    {
        ...
        rwm->beginGroup("Folder");
        rwm->beginGroup(dir_name); // Folder name
        rwm->setValue("exec", app_path); // set the favorite application path
        rwm->endGroup(); // Folder name
        rwm->endGroup(); // Folder
        rwm->endGroup(); // Desktop
    }
