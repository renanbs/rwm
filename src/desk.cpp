...
Desk::Desk(Rwm *a, QWidget *parent) : QLabel(parent)
{
...
void Desk::set_desk_icons()
{
    ...
    rwm->beginGroup("Folder");

    for (int i = 0; i < rwm->childGroups().size(); i++)
    {
        rwm->beginGroup(rwm->childGroups().value(i)); // Folder name

        QString name = rwm->value("name").toString();
        QString path = rwm->value("path").toString();
        QRect geom = rwm->value("geometry").value<QRect>();
    ...
    for (int i = 0; i < rwm->childGroups().size(); i++)
    {
        rwm->beginGroup(rwm->childGroups().value(i)); // File name
...
void Desk::run_menu(QAction *act)
{
    ...
    if (act->text().compare(tr("Quit RWM")) == 0)
    {
        qDebug() << "Quit pressed!" << endl;
        app->wm_quit();
    }
    if (act->text().compare(tr("About")) == 0)
    {
        About about(this);
        qDebug() << "eh aqui mesmo!" << endl;
        about.exec();
    }
}
...
void Desk::create_desk_folder(const QString &name, const QString &path, const QRect &geometry, const QPoint &pos, QWidget *parent)
{
    ...
    rwm->setValue ("exec", ""); // for future association with favorite app to open
    rwm->setValue ("pos", pos);
    rwm->setValue ("geometry", geometry);
    rwm->endGroup (); //name
    rwm->endGroup (); //Folder
    rwm->endGroup (); //Desktop
}
...
{
    // remove the deskfolder from desk and from rwm.cfg
    rwm->beginGroup("Desktop");
    rwm->beginGroup("Folder");
    rwm->remove(d_folder->get_dir_name());
    rwm->endGroup(); // Folder
    rwm->endGroup(); // Desktop
...
void Desk::create_desk_file(const QString &name, const QString &path, const QString &icon, const QPoint &pos, QWidget *parent)
{
    ...
    rwm->setValue("exec", ""); // for future association with favorite app to run
    rwm->setValue("pos", pos);
    rwm->endGroup(); //name
    rwm->endGroup(); //File
    rwm->endGroup(); //Desktop
...
{
    // remove the deskicon from desk and from rwm.cfg
    rwm->beginGroup("Desktop");
    rwm->beginGroup("File");
    rwm->remove(d_file->get_file_name());
    rwm->endGroup(); // File
    rwm->endGroup(); // Desktop
...
{
    // remove the deskapp from desk and from Rwm.conf
    rwm->beginGroup("Desktop");
    rwm->beginGroup("App");
    rwm->remove(d_app->get_app_name());
    rwm->endGroup(); // App
    rwm->endGroup(); // Desktop
